...
class Display():
    ...
    def generate_image(self, image, target_class=None):
        ...
        with torch.no_grad():
            ...
            neg_values = np.count_nonzero(cam < 0)
            # count the positive values
            pos_values = np.count_nonzero(cam > 0)
            if neg_values > pos_values:
                cam = -cam
    ...
    def save(self, image, path='visualization', target_class=None, file='network_name'):
        cam = self.generate_image(image, target_class)
        save_class_activation_images(image, cam, file, path)
